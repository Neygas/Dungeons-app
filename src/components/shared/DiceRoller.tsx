...
export default function DiceRoller({ onClose }: Props) {
  ...
  const roll = (sides: number) => {
    ...
    if (isCrit) setTimeout(() => haptic.medium(), 80)
    const modStr = modifier !== 0 ? (modifier > 0 ? `+${modifier}` : `${modifier}`) : ''
    const notation = `${count}d${sides}${modStr}`
    setHistory(prev => [{ notation, rolls, modifier, total, isCrit, isFail }, ...prev].slice(0, 8))
  }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          style={{ background: 'var(--white)', borderRadius: '12px 12px 0 0', width: '100%', maxWidth: 480, padding: '16px 16px 32px', boxShadow: '0 -4px 24px rgba(0,0,0,.15)' }}
        >
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 14, alignItems: 'center' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {[1, 2, 3, 4].map(n => (
                  <button key={n} onClick={() => setCount(n)} style={{ flex: 1, padding: '6px 0', border: `1px solid ${count === n ? 'var(--teal)' : 'var(--border2)'}`, background: count === n ? 'var(--teal-light)' : 'var(--white)', color: count === n ? 'var(--teal2)' : 'var(--text2)', fontSize: 13, fontWeight: 600, borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit' }}>{n}</button>
                ))}
              </div>
            </div>
            <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 6, marginBottom: 16 }}>
            {DICE.map(sides => (
              <button key={sides} onClick={() => roll(sides)} style={{ padding: '10px 0', border: '1px solid var(--border2)', background: sides === 20 ? 'var(--teal)' : 'var(--white)', color: sides === 20 ? '#fff' : 'var(--text)', fontSize: 12, fontWeight: 700, borderRadius: 4, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 1px 3px rgba(0,0,0,.06)', transition: 'all .12s' }}>
                d{sides}
              </button>
            ))}
          </div>
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <AnimatePresence initial={false}>
                  {history.map((r, i) => (
                    <motion.div
                      key={r.notation + r.total + i}
                      initial={{ opacity: 0, y: -12, scale: 0.95 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      transition={{ duration: 0.2 }}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px', background: i === 0 ? 'var(--teal-light)' : 'var(--bg)', borderRadius: 4, border: `1px solid ${i === 0 ? 'var(--teal)' : 'var(--border)'}` }}
                    >
                      ...
                      <span style={{ fontSize: 16, fontWeight: 800, color: totalColor(r), minWidth: 28, textAlign: 'right' }}>{r.total}</span>
